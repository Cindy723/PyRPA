...
def Analysis(PicName, location):
    ...
    while local < Key_Value_pair and running == 1:
        ...
        if NowRowKey[local] == '左键':
            ...
            pyautogui.typewrite(message=str(NowRowValue[local]), interval=0)
    ...
    if PicName != '' and os.path.exists(ImgPath) is True and running == 1:
        ...
        if location is not None:
            ...
            while timeout > 0 and location is None and running == 1:
                ...
                if ViewLog:
                    mylog(ImgPath, 'is not appear,waiting..')
                    ViewLog = False
...
def DataCheck(sheet):
    ...
    def ShowErroInfo():
        ...
        pyautogui.alert(text='！第 ' + str(nowrow+1) + ' 行 ' + LineValue[line] + ' 列 数据有问题，程序无法继续运行', title='AutoWorkMessage')

    def ActionCheck(nowrow):
        Action = str(sheet.row(nowrow)[6].value)
        ReAction = Action.replace(',', '，')
        # mylog(nowrow+1, ' 等号次数', ReAction.count('='), '逗号次数', ReAction.count('，'))
    ...
    for nowrow in range(1, sheet.nrows):
        ...
        for line in range(1, 7):     # 判断各行的各列 程序是以0行开始 为用户显示的是实际表格
            ...
            if line == 1 and stype != 2:
                ShowErroInfo()
                return False
            elif sheet.row(nowrow)[1].value == 1 and sheet.row(nowrow)[2].value != '':    # 检查找图模式已启用的
...
#  @ 功能：清除pyinstaller 打包后运行时产生的临时文件
def ClearTemp():
...
if __name__ == '__main__':
    RunCounter = 0
    StatusText = '启动'
    # time.sleep(0.1) # 避免 base_library.zip 错误
    ...
    # HotKey:
    # KeyAndEn = XlsSource.row(6)[8].value
    # HotKey2 = XlsSource.row(7)[8].value
    # HotKey1 = re.split('/', KeyAndEn)[0]
    # keyboard.add_hotkey(HotKey1, begin_working)
